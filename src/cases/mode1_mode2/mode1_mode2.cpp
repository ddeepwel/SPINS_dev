/* Script for the formation of a mode-1, mode-2 ISWs or a gravity current with:
...
double H1, H2;                  // Total height of mixed region - mode-1, mode-2 (m)
double L1, L2;                  // widths of mixed regions on either tank end - mode-1, mode-2 (m)
// dye parameters
double dye_halfwidth;           // dye transition length (m)
double dye1_loc, dye2_loc;      // dye locations (m)
double dye1_width, dye2_width;  // dye widths (m)
...
class userControl : public BaseCase {
    public:
        ...
        Array<double,1> filt1, filt2, filt3;     // 1D grid vectors
        Array<double,1> rho1, rho2, rho3;        // 1D density profiles
        ...
        void init_tracers(vector<DTArray *> & tracers) {
            ...
            DTArray & dye1 = *tracers[DYE1];
            DTArray & dye2 = *tracers[DYE2];
            
            if (restarting and !restart_from_dump) {
                init_tracer_restart("rho",rho);
                if (is_tracer) {
                    init_tracer_restart("dye1",dye1);
                    init_tracer_restart("dye2",dye2);
                }
            } else if (restarting and restart_from_dump) {
                init_tracer_dump("rho",rho);
                if (is_tracer) {
                    init_tracer_dump("dye1",dye1);
                    init_tracer_dump("dye2",dye2);
                }
            } else {
                // define horizontal extent of each region
                filt1 = 0.5*(1.0-tanh((xx(ii)-L1)/delta_x));
                filt2 = 0.5*(tanh((xx(ii)-L1)/delta_x) - tanh((xx(ii)-(Lx-L2))/delta_x));
                filt3 = 0.5*(1.0+tanh((xx(ii)-(Lx-L2))/delta_x));

                if (is_mapped()) {
                    // density profiles in each region
                    rho1 = -0.5*delta_rho * tanh(((*zgrid)(ii,jj,kk)-(pyc_loc - H1))/h_halfwidth);
                    rho2 = -0.5*delta_rho * tanh(((*zgrid)(ii,jj,kk)-(pyc_loc     ))/h_halfwidth);
                    rho3 = -0.25*delta_rho * (
                              tanh(((*zgrid)(ii,jj,kk)-pyc_loc+0.5*H2)/h_halfwidth)
                             +tanh(((*zgrid)(ii,jj,kk)-pyc_loc-0.5*H2)/h_halfwidth));
                } else {
                    // density profiles in each region
                    rho1 = -0.5*delta_rho * tanh((zz(ii)-(pyc_loc - H1))/h_halfwidth);
                    rho2 = -0.5*delta_rho * tanh((zz(ii)-(pyc_loc     ))/h_halfwidth);
                    rho3 = -0.25*delta_rho * (
                              tanh((zz(ii)-pyc_loc+0.5*H2)/h_halfwidth)
                             +tanh((zz(ii)-pyc_loc-0.5*H2)/h_halfwidth));
                }
                // Define full density field
                rho = filt1(ii) * rho1(kk) + filt2(ii) * rho2(kk) + filt3(ii) * rho3(kk) + 0*jj;

                // Tracer
                if (is_tracer) {
                    dye1 = 0.5*(tanh((xx(ii) - (dye1_loc-0.5*dye1_width))/dye_halfwidth)
                              - tanh((xx(ii) - (dye1_loc+0.5*dye1_width))/dye_halfwidth));
                    dye2 = 0.5*(tanh((xx(ii) - (dye2_loc-0.5*dye2_width))/dye_halfwidth)
                              - tanh((xx(ii) - (dye2_loc+0.5*dye2_width))/dye_halfwidth));
                }
                ...
                if (is_tracer) {
                    write_array(dye1,"dye1",plot_number);
                    write_array(dye2,"dye2",plot_number);
                }
            }
        }
        ...
        void analysis(double time, DTArray & u, DTArray & v, DTArray & w,
            ...
            double max_dye1, max_dye2;
            if (is_tracer) {
                max_dye1 = psmax(max(abs(*tracers[DYE1])));
                max_dye2 = psmax(max(abs(*tracers[DYE2])));
            }
            ...
            if (master()) {
                ...
                add_diagnostic("Max_dye1", max_dye1,    header, line);
                add_diagnostic("Max_dye2", max_dye2,    header, line);
            ...
            if ((time - next_plot) > -1e-6) {
                ...
                if (is_tracer) {
                    write_array(*tracers[DYE1],"dye1",plot_number);
                    write_array(*tracers[DYE2],"dye2",plot_number);
                }
                if (write_pressure)
        ...
        void write_variables(DTArray & u,DTArray & v, DTArray & w,
            ...
            if (is_tracer) {
                write_array(*tracers[DYE1],"dye1.dump");
                write_array(*tracers[DYE2],"dye2.dump");
            }
        }
        ...
        userControl():
            ...
            filt1(split_range(Nx)), filt2(split_range(Nx)), filt3(split_range(Nx)),
            rho1(Nz), rho2(Nz), rho3(Nz),
...
int main(int argc, char ** argv) {
    ...
    add_option("delta_rho",&delta_rho,"Density difference between layers (as percentage of rho_0)");
    add_option("pyc_loc",&pyc_loc,"Pycnocline location");
    add_option("h_halfwidth",&h_halfwidth,"Pycnocline half-width");
    add_option("delta_x",&delta_x,"Horizontal transition half-width");
    add_option("H2",&H2,"Height of mixed region (mode-2)");
    add_option("H1",&H1,"Heigth of mixed region (mode-1)");
    add_option("L1",&L1,"Length of region 1 (mode-2)");
    add_option("L2",&L2,"Length of region 2 (mode-1)");
    add_option("dye1_loc",&dye1_loc,"Location of dye 1");
    add_option("dye2_loc",&dye2_loc,"Location of dye 2");
    add_option("dye1_width",&dye1_width,"Width of dye 1");
    add_option("dye2_width",&dye2_width,"Width of dye 2");
    ...
    N2_max = g*delta_rho/(2*h_halfwidth);
    // Maximum time step
    if (dt_max == 0.0) {
