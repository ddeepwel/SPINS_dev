/* Script for the formation of a mode-2 ISW with:
...
// dye parameters (bottom trapped)
double dye_halfwidth;           // dye transition length (m)
double dye_thickness;           // dye thickness (m)
            ...
            if (restarting and !restart_from_dump) {
                ...
                if (is_tracer) {
                    if (is_mapped()) {
                        ...
                        dye = 0.5*(1.0-tanh((zz(kk) - (topo(ii)+dye_thickness+MinZ))/dye_halfwidth));
                    }
                }
